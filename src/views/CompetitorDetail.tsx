    ...
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-1">
            Last crawled:{' '}
            {biz.lastCrawledAt ? new Date(biz.lastCrawledAt).toLocaleDateString() : 'Never'}
          </p>
        </div>
      ...
      {biz.enrichmentErrors &&
        (biz.enrichmentErrors.crawl ||
          biz.enrichmentErrors.google ||
          biz.enrichmentErrors.serp) && (
      ...
      <div className="card-surface">
        {activeTab === 'seo' && s && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              <p className="metric-label">Internal Links</p>
              <p className="text-lg font-bold">{s.seo.internalLinkCount}</p>
            </div>
            <div className="grid grid-cols-3 gap-4">
              <div>
                <p className="metric-label">Sitemap</p>
                <p className="text-sm font-medium">{s.seo.hasSitemap ? '✓ Yes' : '✗ No'}</p>
              </div>
              <div>
                <p className="metric-label">Robots.txt</p>
                <p className="text-sm font-medium">{s.seo.hasRobotsTxt ? '✓ Yes' : '✗ No'}</p>
              </div>
              <div>
                <p className="metric-label">Alt Tags</p>
                <p className="text-sm font-medium capitalize">{s.seo.altTagCoverage}</p>
              </div>
            </div>
            <div>
              <h3 className="metric-label mb-2">Schema Markup</h3>
              <div className="flex flex-wrap gap-1">
                {s.seo.schemaMarkupTypes.length ? (
                  s.seo.schemaMarkupTypes.map((t) => (
                    <span key={t} className="score-chip bg-muted text-muted-foreground">
                      {t}
                    </span>
                  ))
                ) : (
              ...
              <div>
                ...
                <div className="flex items-center gap-4 mb-4">
                  ...
                  <span className="text-sm text-muted-foreground">
                    ({biz.googleData.reviewCount} reviews)
                  </span>
                </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              {s.trust.accreditations.length > 0 ? (
                <div className="flex flex-wrap gap-1">
                  {s.trust.accreditations.map((a) => (
                    <span key={a} className="score-chip bg-primary/10 text-primary">
                      {a}
                    </span>
                  ))}
                </div>
            ...
            <div>
              ...
              {s.trust.certifications.length > 0 ? (
                <div className="flex flex-wrap gap-1">
                  {s.trust.certifications.map((c) => (
                    <span key={c} className="score-chip bg-muted text-muted-foreground">
                      {c}
                    </span>
                  ))}
                </div>
            ...
            <div>
              ...
              {s.trust.guaranteesMentioned.length ? (
                s.trust.guaranteesMentioned.map((g) => (
                  <p key={g} className="text-sm">
                    • {g}
                  </p>
                ))
              ) : (
                <span className="text-sm text-muted-foreground">None mentioned</span>
              )}
            </div>
          </div>
        )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              {s.content.servicesListed.length > 0 ? (
                <div className="flex flex-wrap gap-1">
                  {s.content.servicesListed.map((sv) => (
                    <span key={sv} className="score-chip bg-muted text-foreground">
                      {sv}
                    </span>
                  ))}
                </div>
            ...
            <div>
              ...
              {s.content.serviceAreasMentioned.length > 0 ? (
                <div className="flex flex-wrap gap-1">
                  {s.content.serviceAreasMentioned.map((a) => (
                    <span key={a} className="score-chip bg-muted text-muted-foreground">
                      {a}
                    </span>
                  ))}
                </div>
            ...
            <div className="grid grid-cols-3 gap-4">
              <div>
                ...
                <p className="text-sm font-medium">{s.content.hasPortfolio ? '✓ Yes' : '✗ No'}</p>
              </div>
              <div>
                <p className="metric-label">FAQ</p>
                <p className="text-sm font-medium">{s.content.hasFAQ ? '✓ Yes' : '✗ No'}</p>
              </div>
              <div>
                <p className="metric-label">Blog</p>
                <p className="text-sm font-medium">{s.content.hasBlog ? '✓ Yes' : '✗ No'}</p>
              </div>
            </div>
            <div>
              ...
              <div className="space-y-2 text-sm">
                <p>
                  {s.engagement.hasBookingSystem
                    ? `✓ Booking: ${s.engagement.bookingProvider}`
                    : '✗ No booking system'}
                </p>
                <p>{s.engagement.hasContactForm ? '✓ Contact form' : '✗ No contact form'}</p>
                <p>
                  {s.engagement.hasNewsletterSignup ? '✓ Newsletter signup' : '✗ No newsletter'}
                </p>
              </div>
            </div>
            <div>
              ...
              {s.engagement.ctaText.length > 0 ? (
                <div className="flex flex-wrap gap-1">
                  {s.engagement.ctaText.map((c) => (
                    <span key={c} className="score-chip bg-primary/10 text-primary">
                      {c}
                    </span>
                  ))}
                </div>
            ...
            <div>
              ...
              {s.engagement.socialLinksPresent.length > 0 ? (
                <div className="flex flex-wrap gap-1">
                  {s.engagement.socialLinksPresent.map((p) => (
                    <span key={p} className="score-chip bg-muted text-muted-foreground">
                      {p}
                    </span>
                  ))}
                </div>
