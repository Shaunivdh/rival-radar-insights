      ...
      if (projectId) {
        await step.run('check-generate-priority-actions', async () => {
          ...
          try {
            ...
            console.log(`[onFailure] Generated ${actions.length} priority actions for project ${projectId}`);
          } catch (e) {
            if (e instanceof AIUnavailableError) {
              await writeEnrichmentError(ownRaw.id, 'ai_actions', `Recommendations unavailable — retrying at ${(e as AIUnavailableError).retryAt}`);
            } else {
