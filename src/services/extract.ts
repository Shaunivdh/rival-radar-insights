...
export async function extractSignals(rawResult: RawCrawlResult): Promise<ExtractedSignals> {
  ...
  return {
    seo: {
      ...
      pageCount: rawResult.pages.length,
    ...
    pricing: {
      hasPricingPage: pickBool(pages, 'hasPricingPage'),
      pricingMentions: mergeObjectArrays(pages, 'pricingMentions') as ExtractedSignals['pricing']['pricingMentions'],
      hasPackages: pickBool(pages, 'hasPackages'),
      packageDetails: mergeObjectArrays(pages, 'packageDetails') as ExtractedSignals['pricing']['packageDetails'],
      hasFreeQuote: pickBool(pages, 'hasFreeQuote'),
      hasFreeTrial: pickBool(pages, 'hasFreeTrial'),
      priceTransparencyScore: (pickStr(pages, 'priceTransparencyScore') as 'high' | 'medium' | 'low' | 'none') || 'none',
    },
    trust: {
      ...
      namedClientsOrPartners: mergeStringArrays(pages, 'namedClientsOrPartners'),
      caseStudyCount: pickNum(pages, 'caseStudyCount', 0) ?? 0,
      testimonialCount: pickNum(pages, 'testimonialCount', 0) ?? 0,
      videoTestimonials: pickBool(pages, 'videoTestimonials'),
      reviewPlatformsLinked: mergeStringArrays(pages, 'reviewPlatformsLinked'),
      trustBadges: mergeStringArrays(pages, 'trustBadges'),
      yearsInBusiness: pickNum(pages, 'yearsInBusiness'),
      teamPageExists: pickBool(pages, 'teamPageExists'),
      namedTeamMemberCount: pickNum(pages, 'namedTeamMemberCount', 0) ?? 0,
    ...
    engagement: {
      hasChatWidget: pickBool(pages, 'hasChatWidget'),
      chatProvider: pickStr(pages, 'chatProvider') || null,
