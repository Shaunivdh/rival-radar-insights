    ...
    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          <DialogContent>
            ...
            <DialogFooter>
              ...
              <Button onClick={handleAdd} disabled={adding} className="bg-primary text-primary-foreground hover:bg-primary/90">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        {[
          ...
          { label: 'Above you', value: active.filter((c) => (c.aiScore?.overallScore ?? 0) > ownScore).length, icon: Trophy, color: 'text-[hsl(142,71%,35%)]' },
          { label: 'Below you', value: active.filter((c) => (c.aiScore?.overallScore ?? 0) <= ownScore).length, icon: Target, color: 'text-[hsl(38,92%,40%)]' },
          ...
          <div key={s.label} className="card-surface flex items-center gap-4">
            <div className={`w-11 h-11 rounded-xl bg-muted/60 flex items-center justify-center ${s.color}`}>
              ...
              <div className="card-surface hover:-translate-y-0.5 transition-transform">
                <div className="flex items-center gap-6 flex-wrap lg:flex-nowrap">
                  ...
                  {c.googleData ? (
                    <div className="flex items-center gap-1 text-sm w-24">
                      ...
                      <span className="text-xs text-muted-foreground">({c.googleData.reviewCount})</span>
                    </div>
                  ) : (
                  ...
                  {/* Score */}
                  <div className={`font-bold text-3xl w-14 text-right ${score !== null ? getScoreColor(score) : 'text-muted-foreground'}`}>
                    {score ?? '—'}
                  </div>
                ...
                {(strengths.length > 0 || weaknesses.length > 0) && (
                  <div className="flex flex-wrap gap-2 mt-4 pt-4 border-t border-border/50">
                    {strengths.map((s) => (
                      <Badge key={s} variant="secondary" className="bg-[hsl(142,71%,35%)]/10 text-[hsl(142,71%,35%)] border-0">
                        ↑ {s}
                      </Badge>
                    ))}
                    {weaknesses.map((w) => (
                      <Badge key={w} variant="secondary" className="bg-destructive/10 text-destructive border-0">
                        ↓ {w}
                      </Badge>
                    ))}
                  </div>
                  ...
                  <div className="card-surface opacity-75 hover:opacity-100 transition-opacity">
                    <div className="flex items-center gap-6 flex-wrap lg:flex-nowrap">
                      <Badge variant="secondary" className="bg-muted text-muted-foreground border-0 gap-1">
                        <Archive className="w-3 h-3" />HISTORIC
                      </Badge>
                      ...
                      <div className="flex-1 min-w-[180px]">
                        ...
                        <p className="text-xs text-muted-foreground flex items-center gap-2 mt-0.5">
                          ...
                          <span>·</span>
                          <Clock className="w-3 h-3" />Archived {archivedAt}
                        </p>
                      </div>

                      {c.googleData && (
                        <div className="hidden md:flex items-center gap-1 text-sm w-24 text-muted-foreground">
                          <Star className="w-4 h-4" />{c.googleData.googleRating}
                          <span className="text-xs">({c.googleData.reviewCount})</span>
                        </div>
                      )}

                      <div className="text-right">
                        <p className="text-[10px] uppercase tracking-wider text-muted-foreground">Last score</p>
