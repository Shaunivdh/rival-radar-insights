import {
  Html, Head, Body, Container, Section, Heading, Text, Button, Hr, Row, Column, Font,
...
const SCORE_LABELS: { label: string; key: keyof Pick<AIHealthScore, 'reputationScore' | 'localVisibilityScore' | 'websiteHealthScore'> }[] = [
...
const styles = {
  ...
  headerTitle: { color: '#ffffff', margin: 0, fontSize: 22, fontWeight: 700 } as React.CSSProperties,
  ...
  sectionLabel: { fontSize: 14, color: '#888', textTransform: 'uppercase' as const, letterSpacing: 1, margin: '0 0 16px' },
  scoreColumn: { textAlign: 'center' as const, padding: '16px', backgroundColor: '#fff', borderRadius: 8, margin: '0 4px', border: '1px solid #e5e7eb' },
  ...
  changeRow: { backgroundColor: '#fff', borderRadius: 8, border: '1px solid #e5e7eb', padding: '12px 16px', marginBottom: 8 } as React.CSSProperties,
  ...
  priorityBox: { backgroundColor: '#f0eeff', borderRadius: 8, padding: '20px', border: '1px solid #d4ccff' } as React.CSSProperties,
  priorityLabel: { fontSize: 13, color: '#5B4EE8', textTransform: 'uppercase' as const, letterSpacing: 1, margin: '0 0 8px' },
  priorityText: { color: '#111', fontSize: 14, margin: 0 } as React.CSSProperties,
  ctaSection: { textAlign: 'center' as const, padding: '32px 0' },
  ctaButton: { backgroundColor: '#5B4EE8', color: '#fff', padding: '12px 28px', borderRadius: 6, fontSize: 14, fontWeight: 600, textDecoration: 'none' } as React.CSSProperties,
...
export default function WeeklyDigest({ businessName, scores, topAction, changes, dashboardUrl }: Props) {
  return (
    <Html>
      <Head>
        {/* Inter v13 — pinned URL; update if Google Fonts changes the path */}
        <Font fontFamily="Inter" fallbackFontFamily="Arial" webFont={{ url: 'https://fonts.gstatic.com/s/inter/v13/UcCO3FwrK3iLTeHuS_fvQtMwCp50KnMw2boKoduKmMEVuLyfAZ9hiA.woff2', format: 'woff2' }} fontWeight={400} fontStyle="normal" />
      </Head>
      <Body style={styles.body}>
        ...
        <Container style={styles.mainContainer}>
          <Section style={styles.greeting}>
            <Heading as="h2" style={styles.greetingTitle}>Weekly Report for {businessName}</Heading>
            <Text style={styles.greetingText}>Here's what changed in your competitive landscape this week.</Text>
          </Section>

          <Hr style={styles.hr} />

          <Section>
            <Heading as="h3" style={styles.sectionLabel}>Your Scores</Heading>
            <Row>
          ...
          {changes.length > 0 && (
            <Section>
              <Heading as="h3" style={styles.sectionLabel}>Competitor Activity</Heading>
          ...
          <Section style={styles.priorityBox}>
            <Heading as="h3" style={styles.priorityLabel}>Top Priority This Week</Heading>
            <Text style={styles.priorityText}>{topAction.action}</Text>
            <Text style={styles.changeText}>{topAction.reason}</Text>
          </Section>

          <Section style={styles.ctaSection}>
            <Button href={dashboardUrl} style={styles.ctaButton}>View Full Report</Button>
          </Section>
